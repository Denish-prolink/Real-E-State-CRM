...
import { useGetSellReport } from '../hooks/useGetReports';
...
export default function SellTab() {
  const { data, isLoading } = useGetSellReport();
    ...
    <div className="flex flex-col gap-6 w-full">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-zinc-950 border border-border rounded-xl p-6 shadow-sm flex flex-col items-center text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground">Total Lifetime Sales</p>
        ...
        <div className="bg-white dark:bg-zinc-950 border border-border rounded-xl p-6 shadow-sm flex flex-col items-center text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground">Total Sell Orders</p>
          <p className="text-2xl font-bold mt-1">{totalOrders}</p>
        </div>
      </div>

      <div className="bg-white dark:bg-zinc-950 border border-border rounded-xl p-6 shadow-sm">
        <h3 className="font-semibold text-lg mb-4">Monthly Sales Breakdown</h3>
        <div className="space-y-3">
          {reports.map((r: { _id: { year: number; month: number }; ordersCount: number; revenue: number }) => (
