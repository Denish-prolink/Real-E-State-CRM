...
import AgencyFormDrawer from '@/pages/agencies/components/AgencyFormDrawer';
import { useUpdateAgency } from '@/pages/agencies/hooks/useUpdateAgency';

import type { AddAgencyPayload } from '@/pages/agencies/types/agency.types';
...
export default function ProfilePage() {
  ...
  const handleUpdateAgency = async (values: AddAgencyPayload) => {
    if (user?.agencyId?._id) {
      const response = await updateAgency({ id: user.agencyId._id, payload: values });
    ...
    <div className="flex-1 space-y-6 p-4 md:p-8 pt-6">
          ...
          <div className="rounded-xl border bg-card text-card-foreground shadow">
            <div className="flex flex-col space-y-1.5 p-6 relative">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-semibold leading-none tracking-tight">Agency Details</h3>
                  <p className="text-sm text-muted-foreground mt-1.5">Information about your agency</p>
                </div>
      ...
      {user?.agencyId && (
        <AgencyFormDrawer 
          ...
          editAgencyId={user.agencyId._id}
        />
      )}
    </div>
  );
