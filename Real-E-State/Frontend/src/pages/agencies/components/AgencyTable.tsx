...
export default function AgencyTable({ agencies, isLoading, onEdit, onDelete, onView, startIndex = 0 }: Props) {
  return (
    <div>
                ...
                <TableRow key={agency._id}>
                  ...
                  <TableCell>
                    ...
                    >
                      ...
                      <div>
                        <TruncatedName name={agency.name || "-"} maxLength={20} className="font-medium text-indigo-600 dark:text-indigo-400 " />
                      </div>
                    </div>
                  </TableCell>

                  <TableCell>{agency.contactNumber || "-"}</TableCell>

                  <TableCell>{agency.gst || "-"}</TableCell>

                  <TableCell>{agency.pan || "-"}</TableCell>

                  <TableCell>{agency.members ?? "-"}</TableCell>
                    ...
                    <div className="flex justify-end gap-2">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-32 rounded-lg">
                          <DropdownMenuItem onClick={() => onView(agency)} className="cursor-pointer">
                            <Eye className="mr-2 h-4 w-4" />
                            View
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => onEdit(agency)} className="cursor-pointer">
                            <Edit2 className="mr-2 h-4 w-4" />
                            Edit
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => onDelete(agency._id)} className="cursor-pointer text-red-600 focus:text-red-600">
