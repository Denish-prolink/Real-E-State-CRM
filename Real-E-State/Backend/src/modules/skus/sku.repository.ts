...
export const createSku = async (payload: ISkuPayload & { agencyId: string | undefined }) => {
...
export const countSkus = async (agencyId: string | undefined, search?: string) => {
  return Sku.countDocuments(buildFilter(agencyId, search));
};

export const findSkuById = async (id: string, agencyId: string | undefined) => {
  return Sku.findOne({ _id: id, agencyId });
};

export const findSkuByCode = async (skuCode: string, agencyId: string | undefined) => {
  return Sku.findOne({ skuCode, agencyId });
};

export const updateSku = async (id: string, payload: Partial<ISkuPayload>, agencyId: string | undefined) => {
  return Sku.findOneAndUpdate({ _id: id, agencyId }, payload, { new: true });
};

export const deleteSku = async (id: string, agencyId: string | undefined) => {
